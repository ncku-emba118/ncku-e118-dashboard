...
@media print {
  ...
  /* === 對比強化 v2：手機看 PDF 不糊 === */
  /* signoff-sheet 內所有 div 預設字色加深成黑（蓋掉所有 inline 灰字）*/
  .signoff-sheet, .signoff-sheet * {
    color: #1A1612 !important;
  }
  /* 簽核格職務 label 用酒紅（保留品牌色）*/
  .signoff-grid-print > div > div > div:first-child {
    color: #6B1622 !important;
    font-weight: 700 !important;
  }
  /* 區塊標題列維持酒紅字 */
  .signoff-sheet [style*="background: rgb(244, 239, 230)"],
  .signoff-sheet [style*="background:#F4EFE6"] {
    color: #6B1622 !important;
  }
  ...
  .signoff-sheet [style*="grid-template-columns: 1fr 1fr"] > div > div > span {
  ...
  /* A 框白字、C 框黑字（蓋掉 .signoff-sheet * 黑字 rule） */
  .signoff-sheet > div > div:nth-child(2) > div:nth-child(1),
  .signoff-sheet > div > div:nth-child(2) > div:nth-child(1) * { color: #fff !important; }
  ...
  /* D 班費預備金 row：黑底白字（先前已寫，這裡確保不被新黑字 rule 蓋掉） */
  .signoff-sheet [style*="rgb(255, 248, 231)"],
  .signoff-sheet [style*="rgb(255, 248, 231)"] * { color: #fff !important; background: #1A1612 !important; }
}
`,
        }}
      />
