...
const HELP: Record<string, HelpEntry> = {
  ...
  present: {
    title: '上台簡報模式',
    domain: 'reports.e118.aqualux.dev/present',
    desc: '上台報告用的雲端播放器。教室電腦開瀏覽器就能全螢幕播放已上傳的 PDF，手機掃 QR 後變成遙控器翻頁，不必帶 USB、不必登入、不必在公用電腦裝任何程式。',
    steps: [
      '報告前先把 PDF 上傳到「課程報告」的上傳頁（PPT 請先另存成 PDF）',
      '教室電腦開這張卡 → 選你的簡報 → 按「開始簡報」',
      '畫面會出現 QR Code 與 6 位數字，用手機相機掃它',
      '手機出現大大的「上一頁／下一頁」，按下去教室畫面就換頁',
      '第一支掃到的手機才有控制權，其他人掃只能跟看，不會被亂翻',
      '教室電腦不小心重新整理也沒關係，會自動回到原本那一頁',
    ],
  },
  thesis: {
    title: '論文查詢系統',
  ...
  leads: {
    title: '各活動總召',
    domain: 'emba.aqualux.dev/leads',
    desc: '本班決議每場活動設置總召，依活動性質指派合適幹部、不必固定由活動長擔任。本頁把每場活動的總召依時間序公告出來，讓同學知道該找誰、也讓總召提前有籌備時間。',
    steps: [
      '時間軸由上到下依活動日期排列，最近的一場在最上方',
      '酒紅色實心節點代表尚未舉行的場次，空心代表已過期的場次',
      '每一則列出活動名稱、日期、總召姓名與職稱',
      '人事異動或新增活動：改 lib/leads/data.ts 加一筆，版面自動延長',
      '完整決議內容以幹部會議紀錄為準，會議紀錄公告在班級公告欄',
    ],
  },
  calendar: {
    title: '班級行事曆',
